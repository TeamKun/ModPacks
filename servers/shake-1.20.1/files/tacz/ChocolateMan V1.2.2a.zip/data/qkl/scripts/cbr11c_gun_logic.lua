...
local function getReloadTimingFromParam(param)
    ...
    local intro_empty_clip = param.intro_empty_clip * 1000
...
function M.tick_reload(api)
    ...
    if (reloaded_count >= 0) then
        ...
        if(cache.needed_count == 5 and api:getAttachment("SCOPE") ~= "hamster:gew98_scope") then
            base_time = clip_load_feed
        end
        if (not cache.is_tactical) then
            base_time = base_time + (api:getAttachment("SCOPE") ~= "hamster:gew98_scope" and intro_empty_clip or intro_empty)
        else
        ...
        while (base_time < reload_time) do
            ...
            if (cache.needed_count == 5 and api:getAttachment("SCOPE") ~= "hamster:gew98_scope") then
                reloaded_count = reloaded_count + 6
