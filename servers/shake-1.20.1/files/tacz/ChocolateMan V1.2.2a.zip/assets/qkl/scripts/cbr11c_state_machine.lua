...
function base_track_state.entry(this, context)
    if (context:getAttachment("SCOPE") == "hamster:gew98_scope") then
...
function main_track_states.reload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            if(main_track_states.reload.emptyload == 1 and context:getAttachment("SCOPE") ~= "hamster:gew98_scope") then
                if (main_track_states.reload.loaded_ammo ~= 3) then
                    context:runAnimation("reload_load_clip", track, false, PLAY_ONCE_HOLD, 0.2)
                end
                main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 3
            else
                if (isNoAmmo(context)) then
